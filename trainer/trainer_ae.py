...
class Trainer:
    def __init__(self, config):
        ...
        # model
        self.mem_n2n = model_encdec(self.settings) #model_encdec is not hardcoded
    ...
    def evaluate(self, loader, epoch=0):
        ...
        for step, (index, past, future, presents, angle_presents, videos, vehicles, number_vec, scene, scene_one_hot) \
            ...
            horizon10s += torch.sum(distances[:, 4])
            horizon20s += torch.sum(distances[:, 9])
            horizon30s += torch.sum(distances[:, 14])
            horizon40s += torch.sum(distances[:, 19])
